...
namespace ORB_SLAM
{
    ...
    Tracking::Tracking(ORBVocabulary* pVoc, Map *pMap, string strSettingPath):
    mState(NO_IMAGES_YET), mpORBVocabulary(pVoc),  mpMap(pMap),
    mnLastRelocFrameId(0), mbPublisherStopped(false), mbReseting(false), mbForceRelocalisation(false), mbMotionModel(false)
    {
        ...
        mpORBextractor = new ORBextractor(nFeatures,fScaleFactor,nLevels,fastTh);
        ...
        mpIniORBextractor = new ORBextractor(nFeatures*2,1.2,8,fastTh);
    ...
    void Tracking::GrabImage(cv::Mat &rgb, cv::Mat &depth)
    {
        ...
        assert(rgb.channels()==3 || rgb.channels()==1);
        
        if(rgb.channels()==3)
        {
            if(mbRGB)
                cvtColor(rgb, im, cv::COLOR_RGB2GRAY);
            else
                cvtColor(rgb, im, cv::COLOR_BGR2GRAY);
        }
        ...
        if(mState==WORKING || mState==LOST)
            mCurrentFrame = Frame(im,0,mpORBextractor,mpORBVocabulary,mK,mDistCoef);
        else {
            mCurrentFrame = Frame(im,0,mpIniORBextractor,mpORBVocabulary,mK,mDistCoef);
            //mCurrentFrame.depth = depth;
        }
        ...
        if(!mCurrentFrame.mTcw.empty())
        {
            ...
            pose_R = mCurrentFrame.mTcw.rowRange(0,3).colRange(0,3);
            pose_T = mCurrentFrame.mTcw.rowRange(0,3).col(3);
        }
        
    }
    
    cv::Mat Tracking::GetPose_R() {
        boost::mutex::scoped_lock lock(mMutexPose);
        return pose_R.clone();
    }
    
    cv::Mat Tracking::GetPose_T() {
        boost::mutex::scoped_lock lock(mMutexPose);
        return pose_T.clone();
    }
    ...
    void Tracking::FirstInitialization()
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            //mInitialFrame.depth = mCurrentFrame.depth;
            mLastFrame = Frame(mCurrentFrame);
    ...
    void Tracking::CreateInitialMap(cv::Mat &Rcw, cv::Mat &tcw)
    {
        ...
        //int medianX = -1;
        //int medianY = -1;
        //float medianDepth = pKFini->ComputeSceneMedianDepthWithCoord(medianX, medianY, 2);
        float medianDepth = pKFini->ComputeSceneMedianDepth(2);
        float invMedianDepth;
        
        //indexX = medianX;
        //indexY = medianY;
        //medianDepthLog = medianDepth;
        //depthScale = mInitialFrame.depth.at<float>(medianX, medianY);
        
        //if (medianX != -1 && medianY!= -1 && mInitialFrame.depth.at<float>(medianX, medianY) > 0)
        //    invMedianDepth = mInitialFrame.depth.at<float>(medianX, medianY)/(1000.f*medianDepth);
        //else
