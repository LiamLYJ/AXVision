...
width_pix = (width + 1) * chessBoard_cell + chessBoard_cell  # add extra  chessBoard_cell  for reserve blank
height_pix = (height + 1) * chessBoard_cell + chessBoard_cell
...
for j in range(0,height + 1):
    ...
    for i in range(0,width+1):
        #rint(i)
        x0 = i *chessBoard_cell
        y0 = y
        rect_start = (x0,y0)
 
        x1 = x0 + chessBoard_cell
        y1 = y0 + chessBoard_cell
        rect_end = (x1,y1)
...
cv.imwrite("chessBoard_%d_W_%d_H.jpg"%(width, height),image)
cv.imshow(win_name, image)
